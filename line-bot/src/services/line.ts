...
const
    ...
    lineApiData: AxiosInstance = axios.create({
        baseURL: lineApiDataBase,
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${accessToken}`
        }
...
export async function getMessageImage(messageId: string): Promise<{ buffer: Buffer, contentType: string } | null> {
    try {
        const response = await lineApiData.get(`/message/${messageId}/content`, {
        ...
        console.error('Error getting message image:', (error as any).response ? JSON.stringify((error as any).response.data) : (error as any).message);
        return null;
    }
}